...
def observed_residual(evidence: dict[str, Any]) -> set[str]:
    """The rows nothing in process closes, as measured rather than as declared."""

    return {
        row["id"]
        for row in evidence["mutations"]
        if row["observed_class"] == cat.RESIDUAL
    }
...
def compare_with_declaration(
    ...
    declared_residual = set(declared.get("residual", {})) & selected
    problems: list[str] = []
    ...
    observed_residual_ids = observed_residual(evidence) & selected
    for mutation_id in sorted(observed_residual_ids - declared_residual):
        problems.append(
            f"{mutation_id}: residual observed but not declared in known_escapes.json -- "
            "a residual is an escape closed outside the process, and growing the set "
            "is a regression"
        )
    for mutation_id in sorted(declared_residual - observed_residual_ids):
        problems.append(
            f"{mutation_id}: residual declared but not observed -- either it closed and "
            "the declaration was not updated, or it was not measured"
        )
    for mutation_id in sorted(declared_residual):
        entry = declared.get("residual", {}).get(mutation_id)
        if not isinstance(entry, dict):
            continue
        if not entry.get("argument"):
            problems.append(
                f"{mutation_id}: declared residual with no written argument -- a "
                "residual nobody justified is an escape somebody stopped counting"
            )
        if not entry.get("compensating_controls"):
            problems.append(
                f"{mutation_id}: declared residual with no compensating controls -- a "
                "risk nobody mitigated is not a risk anybody accepted"
            )
...
def closes_with_violations(evidence: dict[str, Any], declared: dict[str, Any]) -> list[str]:
    ...
    problems: list[str] = []
    for mutation_id in sorted(observed_escapes | observed_residual(evidence)):
...
# Roots the measured tree must not be able to reach.  ``app.config`` falls back
# to the packaged deployment root when a tree carries no ``config/``, and M-32
# exists to measure exactly that.  On a machine where the fallback is populated
# the row measures a different thing, so the evidence would depend on which
# machine regenerated it -- which is not evidence.
FALLBACK_ROOTS: tuple[str, ...] = ("/opt/coinalyze",)


def reachable_fallback_roots(roots: tuple[str, ...] = FALLBACK_ROOTS) -> list[str]:
    """Fallback roots that exist on *this* machine and carry a usable config/.

    Checked explicitly and reported by name.  The previous revision assumed the
    auditing machine had none, and an assumption about a machine is not a
    property of the artefact: the frozen evidence is compared byte for byte in
    CI and regenerated by an auditor somewhere else.
    """

    return [
        root for root in roots if (Path(root) / "config").is_dir()
    ]
...
def provisioning_problems(anchor: str, selected: set[str]) -> list[str]:
    ...
    Every condition here fails the *row* closed already, and a row that fails
    closed into an escape which is also a declared escape would otherwise let
    the pipeline exit green over something nobody measured.  That is the shape
    of the empty green this instrument exists to refuse, so the run itself is
    red: the audit gate is not the place to discover the runner was half
    provisioned, or that the machine it ran on changed what was measured.
    """

    problems: list[str] = []
    reachable = reachable_fallback_roots()
    if reachable:
        problems.append(
            "the measured tree can reach a populated fallback root outside the "
            f"harness workspace ({', '.join(reachable)}), so M-32 would measure "
            "that deployment's configuration instead of the absence it exists to "
            "detect; the evidence would not be reproducible on another machine"
        )
...
def emit_known_escapes(evidence: dict[str, Any], revision_label: str) -> str:
    ...
    residual = observed_residual(evidence)
    body = {
        ...
        # Kept in its own section, never merged with the escapes.  An escape is
        # something this tree does not detect yet; a residual is something no
        # process could, so the two are closed by different work and reading one
        # as the other is how a risk stops being tracked.
        "residual": {
            mutation_id: {
                "argument": cat.CATALOG_BY_ID[mutation_id].residual_argument,
                "compensating_controls": list(
                    cat.CATALOG_BY_ID[mutation_id].compensating_controls
                ),
            }
            for mutation_id in sorted(residual)
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"residual observed: {len(residual)} -> {', '.join(sorted(residual)) or '-'}")
    print(f"skipped          : {len(skipped)} -> {', '.join(sorted(skipped)) or '-'}")
    print(
        f"guards           : {len(selected) - len(escapes) - len(skipped) - len(residual)}"
    )
